...
function addSignatureReturns(f) {
  if (navOptions.methodHeadingReturns) {
    ...
    f.signature = "<span class=\"signature\">" + (f.signature || "") +
                  "</span>" + "<span class=\"type-signature\">" +
...
function getPathFromDoclet(doclet) {
  ...
  return path.normalize(doclet.meta.path && doclet.meta.path !== "null" ? doclet.meta.path + "/" + doclet.meta.filename : doclet.meta.filename);
}
...
exports.publish = function(taffyData, opts, tutorials) {
  ...
  function generateQuickTextSearch(tPath, srchDocs, navOps) {
    var templateData = {
      searchableDocuments: JSON.stringify(srchDocs),
      navOptions: navOps
    };

    var tmplString = fs.readFileSync(tPath + "/quicksearch.tmpl").toString(),
      tmpl = _.template(tmplString);

    var html = tmpl(templateData),
